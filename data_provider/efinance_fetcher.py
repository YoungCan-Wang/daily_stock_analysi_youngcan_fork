...
class EfinanceFetcher(BaseFetcher):
    ...
    def __init__(self, sleep_min: float = 1.5, sleep_max: float = 3.0):
        ...
        self.no_proxy = os.getenv("AKSHARE_NO_PROXY", "").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
    ...
    def _fetch_stock_data(
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                logger.warning(
                    f"[API返回] ef.stock.get_quote_history 返回空数据, 耗时 {api_elapsed:.2f}s"
                )
    ...
    def _fetch_etf_data(
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                logger.warning(
                    f"[API返回] ef.fund.get_quote_history 返回空数据, 耗时 {api_elapsed:.2f}s"
                )
